...
class ColorFragment : Fragment() {
    ...
    val seekBarChangeListener = object : SeekBar.OnSeekBarChangeListener {
        ...
        override fun onProgressChanged(seekBar: SeekBar, value: Int, fromUser: Boolean) {
            ...
            when(seekBar) {
                rSeekBar -> { color = Color.rgb(value, gComponent, bComponent) }
                gSeekBar -> { color = Color.rgb(rComponent, value, bComponent) }
                bSeekBar -> { color = Color.rgb(rComponent, gComponent, value) }
            }
